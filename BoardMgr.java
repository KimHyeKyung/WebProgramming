...
public class BoardMgr {
	...
	private static final String  SAVEFOLDER = "D:/javaStudy/workspace/myapp/src/main/webapp/ch15/fileupload";
	...
	public void replyBoard(BoardBean bean) {
		...
		try {
			...
			sql += "values(seq_tblBoard_no.nextval, ?,?,?,?,?,?,sysdate,?,0,?)";
	...
	//파일 다운로드
		public void downLoad(HttpServletRequest req, HttpServletResponse res,
			...
			try {
				...
				if (strClient.indexOf("MSIE6.0") != -1) { //인터넷익스플로어 전용
				...
				if (file.isFile()) {
					BufferedInputStream fin = new BufferedInputStream(		//읽어오기
							new FileInputStream(file));
					BufferedOutputStream outs = new BufferedOutputStream(	//출력
							res.getOutputStream());
					int read = 0;
